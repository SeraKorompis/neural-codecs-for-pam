...
def evaluate_file(audio_path,
    ...
    # # Codec embeddings saved relative to recon_dir parent
    ...
    # Codec embeddings saved to results/reconstruction_quality/codec_embeddings/
    embeddings_dir = Path(PATHS['results_dir']) / 'reconstruction_quality' / \
                     'codec_embeddings' / dataset_name / codec / str(bitrate)
    embeddings_dir.mkdir(parents=True, exist_ok=True)
    embeddings_csv_path = embeddings_dir / 'embeddings.csv'
    ...
    # recon_subdir = config.get('recon_subdir', dataset_name)
    # recon_path   = (recon_dir / recon_subdir / codec /
    ...
    # Resample reconstructed to native SR for metrics
    recon_native = resample(reconstructed, target_sr, native_sr)
    min_len      = min(len(audio), len(recon_native))
    audio_native = audio[:min_len]
    recon_native = recon_native[:min_len]
    audio_duration_sec = round(min_len / native_sr, 4)

    si_snr_whole = si_snr(audio_native, recon_native)
    stft_whole   = stft_distance(audio_native, recon_native, native_sr)
    call_metrics, per_call_df = compute_call_region_metrics(
        audio_native, recon_native, annotations, native_sr
    )

    recon_subdir = config.get('recon_subdir', dataset_name)

    # Save at codec SR → supplementary
    supp_path = (recon_dir.parent / 'supplementary' / 'recon_audio_at_codec_sr' /
                 recon_subdir / codec / str(bitrate) /
                 (audio_path.stem + '_reconstructed.wav'))
    supp_path.parent.mkdir(parents=True, exist_ok=True)
    try:
        save_audio(reconstructed, target_sr, str(supp_path))
    except Exception as e:
        print(f"  Warning: could not save codec SR audio for "
              f"{audio_path.name}: {type(e).__name__}: {e}")

    # Save at native SR → main
    recon_path = (recon_dir / recon_subdir / codec /
                  str(bitrate) / (audio_path.stem + '_reconstructed.wav'))
    recon_path.parent.mkdir(parents=True, exist_ok=True)
    try:
        save_audio(recon_native, native_sr, str(recon_path))
    except Exception as e:
        print(f"  Error: could not save native SR audio for "
              f"{audio_path.name} ({codec} @ {bitrate}): "
              f"{type(e).__name__}: {e}")
        return None, None

    del audio, audio_native, reconstructed, recon_native
...
def run_evaluation(datasets    = None,
    ...
    for dataset_name in datasets:
        ...
        for codec in codecs:
            ...
            recon_subdir = DATASETS[dataset_name].get('recon_subdir', dataset_name)
            print(f"  Recon:    {effective_recon_dir}/{recon_subdir}/{codec}/")
